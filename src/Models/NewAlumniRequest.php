...
class NewAlumniRequest
{
    ...
    /**
     * Ensure the new_alumni_requests table exists.
    ...
    private static function ensureTable(): bool {
        ...
        try {
            ...
            $db->exec("CREATE TABLE IF NOT EXISTS `new_alumni_requests` (
                ...
                `has_alumni_contract` TINYINT(1)    NOT NULL DEFAULT 0          COMMENT '1 = already received alumni contract, 0 = not yet',
            ...
            error_log('[NewAlumniRequest] Failed to ensure table exists: ' . $e->getMessage());
        }
    ...
    public static function getById(int $id): array|false {
        ...
        $db   = \Database::getContentDB();
        $stmt = $db->prepare(
            "SELECT * FROM new_alumni_requests WHERE id = ?"
        );
        $stmt->execute([$id]);
        return $stmt->fetch();
    }
    ...
    public static function getAll(?string $status = null): array {
        ...
        if ($status !== null) {
            $stmt = $db->prepare(
                "SELECT * FROM new_alumni_requests WHERE status = ? ORDER BY created_at DESC"
            );
            $stmt->execute([$status]);
        } else {
            $stmt = $db->query(
                "SELECT * FROM new_alumni_requests ORDER BY created_at DESC"
            );
        }
    ...
    public static function countByStatus(): array {
        ...
        $db   = \Database::getContentDB();
        $stmt = $db->query(
            "SELECT status, COUNT(*) AS cnt FROM new_alumni_requests GROUP BY status"
        );
        $rows   = $stmt->fetchAll();
     ...
     *
     * @param string $newEmail  New e-mail address to look up
    ...
    public static function hasPendingRequest(string $newEmail): bool {
        ...
        $db   = \Database::getContentDB();
        $stmt = $db->prepare(
            "SELECT COUNT(*) FROM new_alumni_requests WHERE new_email = ? AND status = 'pending'"
        );
    ...
    /**
     * Create a new alumni registration request.
     *
     * @param array $data  Keys: first_name, last_name, new_email, old_email,
     *                           graduation_semester, study_program, has_alumni_contract
    ...
    public static function create(array $data): int|false {
        ...
        $db   = \Database::getContentDB();
        $stmt = $db->prepare(
            "INSERT INTO new_alumni_requests
                (first_name, last_name, new_email, old_email, graduation_semester, study_program, has_alumni_contract)
             VALUES (?, ?, ?, ?, ?, ?, ?)"
        );
        ...
        $ok = $stmt->execute([
            ...
            (int) ($data['has_alumni_contract'] ?? 0),
        ]);
    ...
    public static function delete(int $id): bool {
        ...
        $db   = \Database::getContentDB();
        $stmt = $db->prepare("DELETE FROM new_alumni_requests WHERE id = ?");
