...
<!-- Support Modal -->
<div id="supportModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 bug-modal-overlay">
...
// Support Modal
//
// Same containing-block-trap fix as the Ausleihe / Rechnungen modals:
// `position: fixed` only resolves against the viewport when no ancestor
// has `transform`, `filter`, or `perspective`. Profil-Layout hat (z. B.
// Cards mit `transform: translateZ(0)`) genau solche Ancestors, weshalb
// das Modal sonst irgendwo "in der Mitte des Dokuments" landet und der
// User scrollen muss, um es zu sehen. Lösung: das Modal beim Öffnen
// direkt an document.body anhängen (Portal-Pattern).
function showSupportModal(type) {
    const modal = document.getElementById('supportModal');
    if (!modal) return;
    if (modal.parentElement !== document.body) {
        modal.dataset.supportPortal = '1';
    ...
    modal.classList.add('open'); // damit der globale Modal-Observer + body.has-open-modal den Footer ausblendet
