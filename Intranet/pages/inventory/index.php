...
/* ═══ Ausleihen / Entnehmen – Modal ════════════════════════
   Accent: purple/indigo — prefix: inv-
   Inputs excluded from global dark-mode rule via :not() in theme.css
══════════════════════════════════════════════════════════ */
.inv-modal-overlay {
...
.inv-modal-dialog {
    ...
    box-shadow: 0 8px 32px rgba(0,0,0,.14), 0 32px 64px rgba(0,0,0,.12);
    transform: translateY(24px) scale(.96);
    transition: transform .3s cubic-bezier(.22,.68,0,1.2), opacity .25s;
    opacity: 0;
}
.inv-modal-overlay.open .inv-modal-dialog { transform: none; opacity: 1; }
...
.inv-modal-head-title {
    font-size: 1.0625rem; font-weight: 800; color: var(--text-main);
    margin: 0; line-height: 1.2; letter-spacing: -.01em;
}
.inv-modal-head-sub {
    font-size: .775rem; color: var(--text-muted); margin: .15rem 0 0;
...
.inv-modal-close {
    ...
    display: flex; align-items: center; justify-content: center;
    transition: background .15s, color .15s, border-color .15s; font-size: .875rem;
}
.inv-modal-close:hover { background: rgba(239,68,68,.08); border-color: rgba(239,68,68,.4); color: #ef4444; }

/* Body */
.inv-modal-body {
    padding: 1.25rem 1.5rem 1rem; overflow-y: auto; flex: 1;
    display: flex; flex-direction: column; gap: 1rem; scrollbar-width: thin;
}
.inv-form-label {
    display: block; font-size: .725rem; font-weight: 700; color: var(--text-muted);
    text-transform: uppercase; letter-spacing: .06em; margin-bottom: .35rem; line-height: 1.3;
}
.inv-form-input {
    width: 100%; padding: .65rem .9375rem;
...
.inv-form-input::placeholder { color: var(--text-muted); opacity: .55; }
.inv-form-input:focus { border-color: #7c3aed; box-shadow: 0 0 0 3px rgba(124,58,237,.15); background: var(--bg-card); }
.inv-form-input:hover:not(:focus) { border-color: var(--text-muted); }
...
/* Footer */
.inv-modal-foot {
    padding: 1rem 1.5rem 1.375rem; border-top: 1px solid var(--border-color);
    display: flex; gap: .625rem; flex-shrink: 0;
}
.inv-modal-cancel {
    flex: 1; padding: .6875rem 1rem; border-radius: .875rem;
    border: 1.5px solid var(--border-color); background: var(--bg-body);
    color: var(--text-main); font-weight: 600; cursor: pointer;
    font-size: .875rem; transition: background .15s, border-color .15s; text-align: center;
}
.inv-modal-cancel:hover { background: var(--bg-card); border-color: var(--text-muted); }
.inv-modal-submit {
    flex: 2; padding: .6875rem 1rem; border-radius: .875rem;
    background: linear-gradient(135deg, #7c3aed, #2563eb);
...
.inv-modal-submit:hover { opacity: .92; transform: translateY(-1px); box-shadow: 0 6px 20px rgba(124,58,237,.5); }
.inv-modal-submit:active { transform: none; opacity: 1; }
.inv-modal-submit:disabled { opacity: .45; cursor: not-allowed; transform: none; }

/* Mobile: bottom sheet */
@media (max-width: 599px) {
    .inv-modal-overlay { align-items: flex-end; padding: 0; }
    .inv-modal-dialog { width: 100%; max-height: 92dvh; border-radius: 1.5rem 1.5rem 0 0;
        box-shadow: 0 -4px 24px rgba(0,0,0,.15), 0 -16px 48px rgba(0,0,0,.12); }
}
...
.dark-mode .inv-form-input {
    background: rgba(255,255,255,.07);
    border-color: rgba(255,255,255,.11);
    color: var(--text-main);
}
.dark-mode .inv-form-input:focus { background: rgba(255,255,255,.10); border-color: #7c3aed; box-shadow: 0 0 0 3px rgba(124,58,237,.22); }
.dark-mode .inv-form-input:hover:not(:focus) { border-color: rgba(255,255,255,.2); }
.dark-mode .inv-form-input::placeholder { color: rgba(255,255,255,.3); opacity: 1; }
.dark-mode .inv-modal-cancel { background: rgba(255,255,255,.05); border-color: rgba(255,255,255,.11); }
.dark-mode .inv-modal-cancel:hover { background: rgba(255,255,255,.09); border-color: rgba(255,255,255,.2); }
