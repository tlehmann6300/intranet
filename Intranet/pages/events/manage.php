...
/* ── Events Manage – Modals ─────────────────────────────────
   Prefix: emg-modal-   accent: red (delete) / purple (stats)
─────────────────────────────────────────────────────────── */
.emg-modal-overlay {
...
.emg-modal-overlay.open { opacity: 1; pointer-events: auto; }
...
.emg-modal-overlay.open .emg-modal-card { transform: none; opacity: 1; }

.emg-modal-accent { display: block; height: 4px; flex-shrink: 0; border-radius: 1.5rem 1.5rem 0 0; }
.emg-modal-accent--red    { background: linear-gradient(90deg, #ef4444, #dc2626); }
.emg-modal-accent--purple { background: linear-gradient(90deg, #9333ea, #7c3aed); }
...
.emg-modal-title {
    ...
    color: var(--text-main); margin: 0 0 .75rem; line-height: 1.3;
    letter-spacing: -.01em;
}
.emg-modal-text { font-size: .875rem; color: var(--text-muted); line-height: 1.6; margin: 0; }

/* Form fields */
.emg-form-group { display: flex; flex-direction: column; gap: .3rem; margin-bottom: .875rem; }
.emg-form-label {
...
.emg-form-input {
    ...
    border: 1.5px solid var(--border-color); border-radius: .625rem;
...
.emg-form-input::placeholder { color: var(--text-muted); opacity: .6; }
.emg-form-input:focus { border-color: #9333ea; box-shadow: 0 0 0 3px rgba(147,51,234,.12); background: var(--bg-card); }
.emg-form-input:hover:not(:focus) { border-color: var(--text-muted); }
.emg-form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: .75rem; }
@media (max-width: 440px) { .emg-form-grid { grid-template-columns: 1fr; } }
...
.emg-modal-cancel {
    flex: 1; min-width: 7rem; padding: .6875rem 1rem; border-radius: .75rem;
...
.emg-modal-cancel:hover { background: var(--bg-card); border-color: var(--text-muted); }
.emg-modal-action {
    flex: 2; min-width: 7rem; padding: .6875rem 1rem; border-radius: .75rem;
...
.emg-modal-action--red    { background: linear-gradient(135deg, #ef4444, #dc2626); box-shadow: 0 2px 10px rgba(239,68,68,.35); }
.emg-modal-action--purple { background: linear-gradient(135deg, #9333ea, #7c3aed); box-shadow: 0 2px 10px rgba(147,51,234,.35); }
.emg-modal-action:hover { opacity: .92; transform: translateY(-1px); }
.emg-modal-action:active { transform: none; opacity: 1; }

/* Feedback boxes */
.emg-feedback { border-radius: .625rem; padding: .75rem .9rem; font-size: .8125rem; margin-top: .5rem; }
.emg-feedback--error   { background: rgba(239,68,68,.07); border: 1.5px solid rgba(239,68,68,.2); color: #ef4444; }
.emg-feedback--success { background: rgba(34,197,94,.07); border: 1.5px solid rgba(34,197,94,.2); color: #16a34a; }

/* Dark mode — only what CSS vars can't handle */
.dark-mode .emg-modal-card { box-shadow: 0 0 0 1px rgba(255,255,255,.07), 0 24px 64px rgba(0,0,0,.65); }
.dark-mode .emg-form-input {
    background: rgba(255,255,255,.07);
    border-color: rgba(255,255,255,.11);
    color: var(--text-main);
    box-shadow: none;
}
.dark-mode .emg-form-input:focus {
    background: rgba(255,255,255,.10);
    border-color: #9333ea;
    box-shadow: 0 0 0 3px rgba(147,51,234,.22);
}
.dark-mode .emg-form-input:hover:not(:focus) { border-color: rgba(255,255,255,.2); }
.dark-mode .emg-form-input::placeholder { color: rgba(255,255,255,.3); opacity: 1; }
.dark-mode .emg-modal-cancel { background: rgba(255,255,255,.05); border-color: rgba(255,255,255,.11); }
.dark-mode .emg-modal-cancel:hover { background: rgba(255,255,255,.09); border-color: rgba(255,255,255,.2); }

@media (max-width: 600px) {
    .emg-modal-overlay { align-items: flex-end; padding: 0; }
    .emg-modal-card { border-radius: 1.5rem 1.5rem 0 0; max-width: 100%; }
    .emg-modal-accent { border-radius: 1.5rem 1.5rem 0 0; }
}
</style>
...
<div id="deleteModal" class="emg-modal-overlay">
    <div class="emg-modal-card">
        <span class="emg-modal-accent emg-modal-accent--red"></span>
        <div class="emg-modal-body">
            <h3 class="emg-modal-title">
                <i class="fas fa-exclamation-triangle" style="color:#ef4444;margin-right:.5rem;"></i>
                Event löschen
            </h3>
            <p class="emg-modal-text">
                Möchtest Du das Event "<span id="deleteEventName" style="font-weight:700;color:var(--text-main);"></span>" wirklich löschen?
        ...
        <form method="POST" id="deleteForm">
            ...
            <div class="emg-modal-footer">
                <button type="button" id="closeDeleteModalBtn" class="emg-modal-cancel">Abbrechen</button>
                <button type="submit" class="emg-modal-action emg-modal-action--red">
...
<div id="addStatsModal" class="emg-modal-overlay">
    <div class="emg-modal-card" style="max-width:34rem;">
        <span class="emg-modal-accent emg-modal-accent--purple"></span>
        <div class="emg-modal-body">
            <h3 class="emg-modal-title">
                <i class="fas fa-chart-bar" style="color:#9333ea;margin-right:.5rem;"></i>
                Statistiken nachtragen
            </h3>

            <!-- Category -->
            <div class="emg-form-group">
                <label class="emg-form-label">Kategorie</label>
                <select id="statsCategory" onchange="onStatsCategoryChange()" class="emg-form-input">
            ...
            <div id="statsItemFields">
                <div class="emg-form-group">
                    <label class="emg-form-label">Artikelname</label>
                    <input type="text" id="statsItemName" maxlength="255" class="emg-form-input" placeholder="z.B. Bratwurst">
                </div>
                <div class="emg-form-grid">
                    <div class="emg-form-group">
                        <label class="emg-form-label">Menge</label>
                        <input type="number" id="statsQuantity" min="0" step="1" value="0" class="emg-form-input">
                    </div>
                    <div class="emg-form-group">
                        <label class="emg-form-label">Umsatz (€)</label>
                        <input type="number" id="statsRevenue" min="0" step="0.01" class="emg-form-input" placeholder="Optional">
                    </div>
                </div>
                <div class="emg-form-group">
                    <label class="emg-form-label">Jahr</label>
                    <input type="number" id="statsYear" min="2000" max="<?php echo date('Y') + 10; ?>" class="emg-form-input">
                </div>
            </div>

            <!-- Donations field (Spenden) -->
            <div id="statsDonationsField" class="hidden emg-form-group">
                <label class="emg-form-label">Spendenbetrag (€)</label>
                <input type="number" id="statsDonationsTotal" min="0" step="0.01" value="0" class="emg-form-input">
            </div>

            <div id="statsError"   class="emg-feedback emg-feedback--error   hidden"></div>
            <div id="statsSuccess" class="emg-feedback emg-feedback--success hidden"></div>
        </div>

        <div class="emg-modal-footer">
            <button type="button" id="closeAddStatsModalBtn" class="emg-modal-cancel">Abbrechen</button>
            <button type="button" onclick="submitAddStats()" class="emg-modal-action emg-modal-action--purple">
...
/* ── Scroll-lock helpers ──────────────────────────────────────── */
let _emgScrollY = 0;
function _emgLockScroll() {
    _emgScrollY = window.scrollY;
    document.body.style.position = 'fixed';
    document.body.style.top = '-' + _emgScrollY + 'px';
    document.body.style.left = '0';
    document.body.style.right = '0';
    document.body.style.overflow = 'hidden';
}
function _emgUnlockScroll() {
    document.body.style.position = '';
    document.body.style.top = '';
    document.body.style.left = '';
    document.body.style.right = '';
    document.body.style.overflow = '';
    window.scrollTo(0, _emgScrollY);
}
...
function confirmDelete(eventId, eventName) {
    ...
    if (deleteModal) { _emgLockScroll(); deleteModal.classList.add('open'); }
}

function closeDeleteModal() {
    const deleteModal = document.getElementById('deleteModal');
    if (deleteModal) { deleteModal.classList.remove('open'); _emgUnlockScroll(); }
}
...
document.getElementById('addStatsModal')?.addEventListener('click', (e) => {
    if (e.target === document.getElementById('addStatsModal')) closeAddStatsModal();
});
...
function openAddStatsModal(eventId, eventYear) {
    ...
    const m = document.getElementById('addStatsModal');
    if (m) { _emgLockScroll(); m.classList.add('open'); }
}

function closeAddStatsModal() {
    const m = document.getElementById('addStatsModal');
    if (m) { m.classList.remove('open'); _emgUnlockScroll(); }
